...
const slugify = (text, separator = '-') => {
  return text.toString().toLowerCase().trim()
    ...
    .replace(/./g, (target, index, str) => specialsChars[target] || target)
    // Replace spaces, non-word chars and dashes with a single dash (-)
    .replace(/[\s\W-]+/g, '-')
...
const latinTitleCase = (text) => {
  return text.toString()
    .replace(/./g, (target, i, str) => latinCharsMap[target] || target)
    .replace(/[\s\W-]+/g, ' ')
    .replace(/^-+/, '')   // trim dash(-) from start of text
    .replace(/-+$/, '');  // trim dash(-) from end of text
}
